...
if [ $APPNAME == 'AudioPluginHost' ] ; then
	...
	pushd . && cd $BUILDS_DIR/$PLAT_BUILD_DIR && xcodebuild -project "$MACAPPNAME.xcodeproj" -target "$MACAPPNAME - Shared Code" && popd || exit 4
fi
else
	make -C $BUILDS_DIR/$PLAT_BUILD_DIR || exit 4
fi
